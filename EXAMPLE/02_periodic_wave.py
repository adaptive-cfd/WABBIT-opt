...
if __name__ == "__main__":
    ...
    opt_goal = lambda Qmat: -dx*dt*norm(np.abs(Qmat)*np.exp(-0.5*(np.abs(Xgrid)-0.3*L)**2/(0.02*L)**2))**2
    opt_fun_sPOD = lambda mu: opt_goal(np.reshape(my_interpolated_state(sPOD_frames, frame_amplitude_list, mu_vecs, D, Nx, Nt, mu),[Nx,Nt]))
