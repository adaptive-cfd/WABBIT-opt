...
class Decoder(BaseModule):
    def __init__(self, dof=10, f=nn.Sigmoid, spatial_shape=[128, 128], lam=1, init_zero=False):
        ...
        for n in range(len(self.spatial_shape0) - 1):
            ...
            self.unpad.append(
                [m - ((l - 1) * s0 + k) for l, m in zip(self.spatial_shape0[n], self.spatial_shape0[n + 1])])
...
class FTR_AE(BaseModule):
    def __init__(self, encoder=None, decoder=None, dof=10, f=nn.Sigmoid, spatial_shape=[128, 128]):
        ...
        self.encoder = Encoder(dof, spatial_shape=spatial_shape) if encoder is None else encoder
        self.decoder = Decoder(dof, f) if decoder is None else decoder
